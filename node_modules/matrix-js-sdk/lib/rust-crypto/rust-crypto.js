  ...
  saveBackupKeyToStorage() {
    ...
    return _asyncToGenerator(function* () {
      ...
      if (!keyBackupInfo || !keyBackupInfo.version) {
        _this24.logger.info("Not saving backup key to secret storage: no backup info");
        return;
      }
      var backupKeys = yield _this24.olmMachine.getBackupKeys();
      if (!backupKeys.decryptionKey) {
        _this24.logger.info("Not saving backup key to secret storage: no backup key");
        return;
      }
      if (!decryptionKeyMatchesKeyBackupInfo(backupKeys.decryptionKey, keyBackupInfo)) {
        _this24.logger.info("Not saving backup key to secret storage: decryption key does not match backup info");
        return;
      }
  ...
  disableKeyStorage() {
    ...
    return _asyncToGenerator(function* () {
      ...
      if (info !== null && info !== void 0 && info.version) {
        ...
        _this43.logger.error("Can't delete key backup version: no version available");
      }
  ...
  onCryptoEvent(room, event) {
    ...
    return _asyncToGenerator(function* () {
      ...
      if (existingEncryptor) {
        ...
        _this58.roomEncryptors[room.roomId] = new RoomEncryptor(_this58.logger.getChild("[".concat(room.roomId, " encryption]")), _this58.olmMachine, _this58.keyClaimManager, _this58.outgoingRequestsManager, room, config);
      }
    })();
  }
