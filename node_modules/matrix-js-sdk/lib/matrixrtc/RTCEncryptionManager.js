...
export class RTCEncryptionManager {
  ...
  onEncryptionKeysChanged, parentLogger) {
    ...
    _defineProperty(this, "onNewKeyReceived", (userId, deviceId, keyBase64Encoded, index, timestamp) => {
      var _this$logger3;
      (_this$logger3 = this.logger) === null || _this$logger3 === void 0 || _this$logger3.debug("Received key over transport ".concat(userId, ":").concat(deviceId, " at index ").concat(index));
      ...
      if (!outdated) {
        ...
        var _this$logger4;
        (_this$logger4 = this.logger) === null || _this$logger4 === void 0 || _this$logger4.info("Received an out of order key for ".concat(userId, ":").concat(deviceId, ", dropping it"));
      }
    });
  ...
  join(joinConfig) {
    var _this$logger5, _joinConfig$useKeyDel, _joinConfig$keyRotati;
    (_this$logger5 = this.logger) === null || _this$logger5 === void 0 || _this$logger5.info("Joining room");
  ...
  ensureKeyDistribution() {
    if (this.currentKeyDistributionPromise == null) {
      var _this$logger6;
      (_this$logger6 = this.logger) === null || _this$logger6 === void 0 || _this$logger6.debug("No active rollout, start a new one");
      // start a rollout
      this.currentKeyDistributionPromise = this.rolloutOutboundKey().then(() => {
        var _this$logger7;
        (_this$logger7 = this.logger) === null || _this$logger7 === void 0 || _this$logger7.debug("Rollout completed");
        this.currentKeyDistributionPromise = null;
        if (this.needToEnsureKeyAgain) {
          var _this$logger8;
          (_this$logger8 = this.logger) === null || _this$logger8 === void 0 || _this$logger8.debug("New Rollout needed");
      ...
      (_this$logger9 = this.logger) === null || _this$logger9 === void 0 || _this$logger9.debug("Rollout in progress, a new rollout will be started after the current one");
  ...
  onMembershipsUpdate() {
    ...
    (_this$logger0 = this.logger) === null || _this$logger0 === void 0 || _this$logger0.trace("onMembershipsUpdate");
    ...
    return _asyncToGenerator(function* () {
      ...
      if (anyLeft.length > 0) {
        ...
        if (keyAge < _this.keyRotationGracePeriodMs) {
          ...
          (_this$logger1 = _this.logger) === null || _this$logger1 === void 0 || _this$logger1.debug("New joiners detected, but the key is recent enough (age:".concat(keyAge, "), keeping it"));
          toDistributeTo = anyJoined;
          outboundKey = _this.outboundSession;
        } else {
          var _this$logger10;
          // We need to rotate the key
          (_this$logger10 = _this.logger) === null || _this$logger10 === void 0 || _this$logger10.debug("New joiners detected, rotating the key");
      ...
      try {
        var _this$logger11, _this$logger12;
        (_this$logger11 = _this.logger) === null || _this$logger11 === void 0 || _this$logger11.trace("Sending key...");
        ...
        (_this$logger12 = _this.logger) === null || _this$logger12 === void 0 || _this$logger12.trace("key index:".concat(outboundKey.keyId, " sent to ").concat(outboundKey.sharedWith.map(m => "".concat(m.userId, ":").concat(m.deviceId)).join(",")));
        if (hasKeyChanged) {
          var _this$logger13, _this$logger14;
          ...
          (_this$logger13 = _this.logger) === null || _this$logger13 === void 0 || _this$logger13.trace("Delay Rollout for key:".concat(outboundKey.keyId, "..."));
          yield sleep(_this.useKeyDelay);
          (_this$logger14 = _this.logger) === null || _this$logger14 === void 0 || _this$logger14.trace("...Delayed rollout of index:".concat(outboundKey.keyId, " "));
          _this.addKeyToParticipant(outboundKey.key, outboundKey.keyId, getParticipantId(_this.userId, _this.deviceId));
        }
      } catch (err) {
        var _this$logger15;
        (_this$logger15 = _this.logger) === null || _this$logger15 === void 0 || _this$logger15.error("Failed to rollout key", err);
      }
    })();
  }
  createNewOutboundSession() {
    ...
    (_this$logger16 = this.logger) === null || _this$logger16 === void 0 || _this$logger16.info("creating new outbound key index:".concat(newOutboundKey.keyId));
